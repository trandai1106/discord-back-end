...
router.get("/message/:room_id", async (req, res) => {
...
// Create channel
router.post("/create", authMiddleware.requireLogin, async (req, res) => {
  try {
...
router.get("/users/members/:roomId", async (req, res) => {
  try {
    ...
    const result = users.filter((user) => chatRoom.members.includes(user._id));
...
router.get("/users/notmember/:roomId", async (req, res) => {
  try {
    const roomId = req.params.roomId;
...
router.get("/users/search/members/:roomId", async (req, res) => {
  try {
    const roomId = req.params.roomId;
...
router.get("/users/search/notmember/:roomId", async (req, res) => {
  try {
    const roomId = req.params.roomId;
    const query = req.query.q;
...
router.post("/users/add/:roomId", async (req, res) => {
  try {
    const roomId = req.params.roomId;
...
//         return res
//           .status(403)
//           .send("Bạn không phải là thành viên của nhóm chat");
//       }
